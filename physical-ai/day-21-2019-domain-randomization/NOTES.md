...
> 以下 3 组问答来自 2026-09-13 上午用户对 Day21 DR/ADR 卡片的追问（quiz 三题），原样归档。

### Q1：DR 的数学是"鲁棒优化"还是"贝叶斯平均"？transfer 的精确条件是什么？

**问题原文**："DR 的数学是\"鲁棒优化\"（max-min）还是\"贝叶斯平均\"（期望）？transfer 发生的精确条件是什么？"

**核心答案**："贝叶斯平均"是主答案：

$$J_{\text{DR}} = \mathbb{E}_{\xi \sim p_\phi}[J(\theta; \xi)]$$

- $\xi$ ：物理参数向量（摩擦、质量、阻尼等）——"真相"；
- $p_\phi$ ：随机化分布，由超参 $\phi$ 决定——充当先验；
- $\theta$ ：策略参数；
- $J(\theta; \xi)$ ：在参数 $\xi$ 的世界里策略 $\theta$ 的回报。

**transfer 精确条件**： $\xi_{\text{real}} \in \text{support}(p_\phi)$ ，且附近有足够密度——用户原话"先验盖住真相"完全正确。

但"鲁棒优化"不是错答案，两者是**分布宽度的插值**： $p_\phi$ 窄 → 贝叶斯平均；把 $p_\phi$ 拉得很宽，期望就退化成软 max-min——策略被迫"在支撑集里最坏情况也别太烂"，这正是 hedging（魔方 60% 而不是 99% 的来源）。两种失败模式各对应一种解读用错地方：**先验拍错**（贝叶斯失败，真机掉出支撑集）vs **分布太宽吃掉峰值**（鲁棒过头）。考试时答"贝叶斯平均为主，宽分布下退化为软鲁棒"，满分。

### Q2：ADR 的边界扩张为什么是 curriculum？为什么 LSTM 策略会涌现 meta-learning？

**问题原文**："ADR 的边界扩张为什么是一个 curriculum？为什么（LSTM）策略在 ADR 下会涌现 meta-learning？"

**核心答案**：

**curriculum**：熵 $H[p_\phi] = \sum_i \log(hi_i - lo_i)$ 随边界扩张单调增（ $hi_i, lo_i$ 为第 $i$ 个参数区间的上下界），难度永远比能力高一格——这就是 curriculum。

**"hidden state 是对 $\xi$ 的在线后验"**：ADR 每个 episode 采样一个**固定**的 $\xi$ （比如摩擦=0.3），但 policy **看不到** $\xi$ ，只能看到观测 $o_t$ 。想走好，它必须先回答"我在哪个世界里？"——证据全在历史里：发出力矩 $\tau$ ，观测到关节速度 $\omega$ 比预期小 → 摩擦大或负载重；几个 timestep 下来，历史 $h_t = (o_1, a_1, \dots, o_t)$ 里就藏着 $\xi$ 的全部证据。

LSTM 干的事： $m_t = f_\theta(m_{t-1}, o_t, a_{t-1})$ ，把历史压缩成一个向量（ $m_t$ 为 LSTM 隐状态），policy 按 $\pi(a \mid o_t, m_t)$ 行动。它的**功能角色** = 后验 $p(\xi \mid h_t)$ ：把"到目前为止身体感受到的一切"压缩成"对环境的信念"，再基于信念行动。贝叶斯后验是 $p(\xi \mid h) \propto p(h \mid \xi) p(\xi)$ ；LSTM 不显式算 Bayes 公式，但 ADR 训练（ $\xi$ 一直在变 + reward 只奖励"适应对了的"行为）**逼着** $m_t$ 成为 $\xi$ 的充分统计量——"在线后验"说的就是这个功能等价，不是说它手算贝叶斯。

**为什么叫 emergent meta-learning**：meta-learning = "学会如何适应"。外层 PPO 在 $\xi$ 变化的 episodes 上训练，学的不是"某个 $\xi$ 下的步态"，而是 LSTM 的**更新规则** $f_\theta$ 本身——即"如何从历史中推断 $\xi$ "。部署时权重一动不动，适应全靠 $m_t$ 更新 → 学会了"适应"这件事。而它是被 ADR **逼出来**的： $\xi$ 不变 → 没必要辨识；难度不递增 → 躺平学个保守步态（hedging 解）就够了。只有"环境一直在变 + 越来越难"，policy 才被迫"先辨识、再行动"。这正是 Day22 RMA 要**显式化**的东西——Day21 是隐式版。

### Q3：DR vs RLPD——什么时候"把仿真做宽"，什么时候"直接在真机上学"？

**问题原文**："DR vs RLPD 两条相反的路——什么时候\"把仿真做宽\"，什么时候\"直接在真机上学\"？"

**核心答案**：最锋利的一刀——**接触复杂度本身不决定方向，决定方向的是"仿真器结构对不对"**：

- **结构对、参数错**（MuJoCo 的接触模型 OK，只是摩擦/质量测不准）→ DR，把参数做宽盖住真相；
