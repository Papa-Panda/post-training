...
### Q1：SFT + RS + DPO 是啥？（用户，10:14 PDT）

**核心答案**：Day 10 后训练数据引擎的三个齿轮。先钉符号： $\pi_\theta$ = 正在训的模型， $\pi_{\text{ref}}$ = 参考模型（通常是上一轮 SFT 模型）， $r_\phi(x,y)$ = RM 给 (prompt $x$ ，响应 $y$ ) 的打分， $\beta$ = KL 惩罚系数（Llama 取 0.1）。

- **SFT**：给定 (prompt, 好回答) 对，最小化 $-\mathbb{E}[\log \pi_\theta(y|x)]$ 。信号 dense（每 token 都有梯度），但上限被示范质量卡死——模型学不会比示范更好的东西。
- **RS**：每 prompt 采样 $K$ 个响应（论文 $K$ 通常 10–30），用 $r_\phi$ 打分只留最高分进 SFT——"用模型自己的裁判提纯数据"，解决人工写不出那么多好回答的瓶颈。
- **DPO**：在 $(x, y_w, y_l)$ 上直接优化，loss $= -\mathbb{E}[\log \sigma(\hat r(x,y_w)-\hat r(x,y_l))]$ ，隐式奖励 $\hat r(x,y) = \beta\log\frac{\pi_\theta(y|x)}{\pi_{\text{ref}}(y|x)}$ ； $\beta\log(\pi_\theta/\pi_{\text{ref}})$ 天然带 KL 约束。
- **为什么循环**：每轮 $\pi_\theta$ 变强 → 下轮采样质量更高 → RM 挑出的数据更难 → "模型变强 → 数据变难"的自举课程（涌现的 curriculum）。§4.1.6 原文 "we apply the above methods in six rounds"。
- **为什么不用 PPO**：引言 L111–113——PPO 在线 rollout + value 网络 + KL 难控，"tend to be less stable and harder to scale"；SFT+RS+DPO 全 offline，数据可攒、可审、可复用。
- **可执行纪律**：RS 的数据门（RM/judge）每轮必须用 held-out 人工抽检校准一致率（< 80% 则停轮、先回炉 RM），防 RM 偏见 6 轮迭代放大。

### Q2：思考题 (a)①——为什么"RM 高分正例"和"小 gap 难对"不能是同一批数据？（用户先给直觉，助手补数学骨架）

用户直觉：高分例子要质量特别高，小 gap 难对要质量接近，所以大概率不会是高分。

数学骨架：记 $r_w = \hat r(x,y_w)$ ， $r_l = \hat r(x,y_l)$ 。一个 pair 有两个自由度，做 45° 旋转：质量水位 $m = (r_w+r_l)/2$ ，落差 $g = r_w - r_l$ （即 gap）。

1. **正交坐标**：RS 筛的是 $m$ （只筛 $r_w$ 那一端，要冲顶）；DPO-Gap 筛的是 $g$ （要小而为正）。一个响应得分再高，也只字不提它和舞伴的落差——两个过滤器活在不同坐标轴上。
2. **梯度权重**：DPO 单 pair 梯度权重 $\sigma(-g) = 1/(1+e^{g})$ ， $g$ 越小权重越大。RS 顶分 $y_w$ 配普通 $y_l$ → $g$ 大 → $\sigma(-g)\approx 0$ ，对 DPO 贡献几乎为零。RS 正例的"顶"结构性地导致它在 DPO 里的"废"。
3. **交集 = 噪声角**： $m$ 顶 + $g$ 小的交集是 $(r_w,r_l)$ 平面右上角贴对角线的一小块 = 两个都极好的回答、偏好靠抛硬币——全池标注分歧最大的地方。DPO 在此训练学的是标注噪声，不是信号。

结论：真正教得动模型的难对住在**中等水位、小落差**区——模型在那里真分不清，而不是标注员随手。

### Q3：思考题 (a)②—— $N$ 个响应的消费顺序：先 SFT 筛选，再 pair 构造；顺序不能反（用户判断，助手给硬理由）

硬理由：**DPO 的 gap 量在一个"还没出厂的坐标系"里**。DPO loss 每一项的分母 $\pi_{\text{ref}}$ 就是本轮 SFT 之后的模型； $g = \hat r_w - \hat r_l$ 只有在 SFT 锚定 $\pi_{\text{ref}}$ 后才有定义。先造 pair = 用上一轮旧 $\pi_{\text{ref}}$ 量的 gap，SFT 一更新坐标系平移，"小 gap"可能不再是小 gap。

论文 §4.2.1 "双速消费"的依赖含义：RM 用全历史 = 外部裁判帧 $r_\phi$ 稳定、与当前策略解耦 → RS 可先行；DPO 只用最新 batch = gap 策略相对，帧是本轮 SFT 后的 $\pi_{\text{ref}}$ → 必须后行。慢消费者提供稳定的"先"，快消费者依赖新鲜的"后"——这是数据依赖图，不可反向。

两个次理由：① 去噪副作用——SFT 先拿走顶部 $k$ 个，pair 挖掘池自动避开"双顶贴对角线"噪声角；② 梯度不打架——不交消费避免同一 $y$ 在 SFT 被推高、在 DPO 当 $y_l$ 被压低。

### Q4：思考题 (a)③——500k 池的预算分配：训练前可算的 proxy（用户提议 proxy 小模型 sweep，助手补两层结构）

用户方案：小模型 + 不同配比分别训练，同一 eval 下测试——这是**第二层（标定层）**，正确但只解决常数。

**第一层（训练前可算，每轮重算）**：每轮开工前 $n_0$ 个 prompt × $K_0$ 响应做 pilot， $r_\phi$ 打分、算隐式奖励，得两个数：

- $p_{\text{SFT}}$ ：best 响应过质量线 $\tau$ 的 prompt 占比（RS 良品率）；低 → 策略弱 → 预算大头给 SFT， $K$ 开大。
- $q_{\text{DPO}}$ ：pair 中 gap 落在信息带 $[g_{\min}, g_{\max}]$ 的占比（DPO 可挖矿量）；下限排除标签噪声角，上限排除 $\sigma(-g)\approx 0$ 的死 pair。

分配规则**边际回报拉平**： $\frac{\alpha}{1-\alpha} \approx \frac{p_{\text{SFT}}\cdot v_{\text{SFT}}}{q_{\text{DPO}}\cdot v_{\text{DPO}}}$ ， $\alpha$ = SFT 采样预算占比。

第二层标定 $v_{\text{SFT}}, v_{\text{DPO}}$ （"一个好 demo / 一个好 pair 值多少"）用 1B 小模型 × 2–3 种配比跑一次即可。动态含义：早期轮策略弱 → $\alpha$ 大（SFT-heavy）；后期轮 gap 收窄进信息带 → $\alpha$ 下降转 DPO。答案不是数字，是**控制器**—— $\alpha$ 的 schedule 就是 basin 距离的度量。

### Q5："405B 自举失效"具体指什么？出自哪篇论文？（用户，11:33 PDT；原文已核对）

出自 **Llama 3 Herd 论文**（arXiv 2407.21783）§4.3.1 "Synthetic data generation: execution feedback"。原文：
...
背景：code expert 的合成 SFT 数据（2.7M 中的约 1M 走此路线）——模型自己出题、自己写解。8B/70B 吃 405B 生成的数据涨点明显（向下蒸馏）；405B 吃自己的生成数据无用甚至掉点。解法：引入模型之外的真值源 execution feedback——出题 → 写解 → parser/linter 静态检查 + 自出 unit test 容器执行 → 失败则带 stdout/stderr/return code 迭代修正 → 全过才进 SFT；约 20% 解法初始错误被纠回。

机制（转述，非论文原话）：向下蒸馏成立因 teacher 分布有 student 没有的信息（净流入）；405B 自采样分布即自身已表示的分布，信息净流入为零，且自身系统性错误模式会被当示范强化——即 "can even degrade" 的来源。注意论文措辞是 "initial experiments revealed"，是作为引入 execution feedback 的动机，不是一次大规模消融。图谱位置："自举有规模上限"在工业论文中的最早书面证据。
