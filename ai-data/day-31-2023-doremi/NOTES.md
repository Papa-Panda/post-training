...
> 来自 `ai data` side chat 用户主动问答（2026-09-08 09:46 PDT），归档时精简整理；答案保留原符号定义与结论。

**问题（用户）**：a) 过滤和配比谁的边际贡献更大？DoReMi 是先切开学权重的，所以猜配比影响更大？ b) "先过滤再配比"是合理的工程决策吗？毕竟过滤的效果可以单独测出来。

**核心答案**：

(a) "谁更大"有个更锋利的切法。先定义：域划分 $D = \{d_1, \dots, d_K\}$ ，域内分布 $P(x|d)$ ，域间权重 $w_d = P(d)$ 。**过滤是域内操作**（改 $P(x|d)$ ，把域内的烂样本拿掉），**配比是域间操作**（改 $w_d$ ，域之间怎么分 token）——配比永远修不好域内的噪声，过滤也永远修不好域间失衡。谁的边际贡献大，取决于池子里哪个是 binding constraint。对合成题池（典型问题是域内噪声而非某域 token 太多），押过滤的边际更大。2×2 预测：(开，DoReMi) ≥ (开，均匀) > (关，DoReMi) > (关，均匀)；关键的一对是中间两个——(开，均匀) vs (关，DoReMi)，这是"样本级过滤"和"域级重加权"作为**去噪替代品**的正面对决。

但这个 2×2 设计本身有个漏洞：**DoReMi 会吃掉过滤的功劳**。DoReMi 的 excess loss 是 $e_d = \ell(\mathrm{proxy 在\ } d) - \ell(\mathrm{专训\ } d \mathrm{的\ ref\ 在\ } d)$ ；烂域里连 ref 都学不好， $e_d$ 小，权重自然被压低——所以 (关，DoReMi) 这一格里，"配比"的贡献已经包含了一部分去噪，测出来的"配比边际"会被系统性高估。修正办法：DoReMi 的 domain 必须切得和过滤正交（按来源×可执行性切，而不是按"质量"切），或者把总增益拆成：过滤主效应 + 配比主效应 + 交互项。交互项为负（替代关系）时，"谁更大"本身就是顺序依赖的伪问题。

(b) "先过滤"的工程直觉是对的，而且可以比"能单独测出来"说得更硬：① 噪声污染一切下游估计——DoReMi 的 proxy 在池子上训出来，池子越脏，excess loss 估计越不准，学出的权重越不可信，先过滤 = 先给估计器降噪；② 成本不对称——过滤大多局部、便宜、可并行（dedup、heuristic），配比学习全局、贵（要训 proxy），便宜的先做；③ 可单独测量。但 caveat：**过滤不是配比中性的**——line-dedup 会系统性砍掉 code 域的高频样板（license、import），过滤完 $P(d)$ 已经变了，code 域可能凭空缩水。正确流程不是"过滤→配比→收工"，而是：过滤 → 重测域直方图 → 在过滤后的池子上学权重 →（可选）再一轮轻过滤；一轮就够，因为过滤对 excess loss 的影响是二阶的。

**修正说明（2026-09-08，同轮对话内）**：昨日思考题题干中"过滤后该域 excess loss 下降、权重被系统性低估"的断言过强，现收回一半——过滤后 proxy 和 ref 的 loss 都下降，gap 的符号不确定；"过滤与配比不交换"成立，但偏差方向不定。原文其余部分不做改动。

**关联**：Day 31 DoReMi（domain-level allocation）。
**图谱关系**：接 Day 24 D4（域内去重）、Day 25 FineWeb（网页过滤）、Day 04 LESS（样本级目标感知选择）——DoReMi 位于"过滤"和"选择"之间的配比层；与 Day 07 / Day 08 / Day 09 报告中的经验配比是一脉，"把人工经验配比改造成可由小模型学习的数据配方"。
